...
public partial class RowMergeView : DataGridView
{
    ...
    protected new void OnCellClick(DataGridViewCellEventArgs e)
    {
        //base.OnCellClick(e);
    }
    protected new void ColumnHeaderMouseClick(DataGridViewCellMouseEventArgs e)
    {
        MessageBox.Show("点击排序");
    }
    #endregion
    ...
    {
        public SpanInfo(int Position, int Left, int Right)
        {
    ...
    public void AddSpanHeader(int ColIndex, int ColCount, string Text)
    {
        ...
        SpanRows[ColIndex] = new SpanInfo(1, ColIndex, Right); //添加标题下的最左列
        SpanRows[Right] = new SpanInfo(3, ColIndex, Right); //添加该标题下的最右列
        for (int i = ColIndex + 1; i < Right; i++) //中间的列
        {
            SpanRows[i] = new SpanInfo(2, ColIndex, Right);
        }
    }
